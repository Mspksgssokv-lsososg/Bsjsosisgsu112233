...
exports.command = async function ({ bot, message, msg, chatId, args }) {
  ...
  if (commandText.length === 0) {
    if (prefixUsed) return message.reply(texts.emptyCommand);
    return;
  }

  let commandArgs = commandText.split(/\s+/);
  ...
  if (!command) {
    if (prefixUsed) {
      return message.reply(
        texts.commandNotFound.replace('{commandName}', commandName)
      );
    }
    return;
  }

  const requiredPrefix = command.config.prefix;
  if (requiredPrefix === false && !prefixUsed) return;
  if (requiredPrefix === true && !prefixUsed) {
    commandText = text.trim();
    commandArgs = commandText.split(/\s+/).slice(1);
  }
  if (requiredPrefix === true && prefixUsed) {
  ...
  const usages = () => {
    if (!command.config.guide) return;
    let usageText = `${symbols} Usages:\n\n`;
    const displayPrefix = command.config.prefix === false ? effectivePrefix : "";
    if (Array.isArray(command.config.guide)) {
      usageText += command.config.guide
        .map(guide => `${displayPrefix}${command.config.name} ${guide}`)
        .join("\n");
    } else {
      usageText += `${displayPrefix}${command.config.name} ${command.config.guide}`;
    }
  ...
  if (command.config.type === "administrator" && !isBotAdmin) {
    if (!["group", "supergroup"].includes(chat.type)) {
      return message.reply(
        texts.adminGroupOnly.replace("{commandName}", command.config.name)
      );
    }
    try {
      ...
      if (!(member.status === "administrator" || member.status === "creator")) {
        return message.reply(
          texts.notGroupAdmin.replace("{commandName}", command.config.name)
        );
      }
    } catch {
  ...
  if (command.config.type === "admin" && !isBotAdmin) {
    return message.reply(
      texts.notBotAdmin.replace("{commandName}", command.config.name)
    );
  }

  if (command.config.type === "vip" && !isVIP) {
    return message.reply(
      texts.notVip.replace("{commandName}", command.config.name)
    );
  }

  if (command.config.type === "group" && !["group", "supergroup"].includes(chat.type)) {
    return message.reply(
      texts.groupOnly.replace('{commandName}', command.config.name)
    );
  }

  if (command.config.type === "private" && chat.type !== "private") {
    return message.reply(
      texts.privateOnly.replace('{commandName}', command.config.name)
    );
  }
  ...
  const expirationTime = (command.config.cooldown || 1) * 1000;
...
Execution : ${cyan}${executionTime}ms${reset}
${separator}
`.trim();
